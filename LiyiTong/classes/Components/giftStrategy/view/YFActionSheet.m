...
#import "YFActionSheetItem.h"

@interface YFActionSheet ()
...
@implementation YFActionSheet

static YFActionSheet *sheet = nil;
...
- (instancetype)initWithTitles:(NSArray *)titles iconNames:(NSArray *)iconNames
{
    ...
    if (self) {
        ...
        for (int i = 0; i < count ; i++) {
            YFActionSheetItem *item = [[YFActionSheetItem alloc] initWithFrame:CGRectMake(itemX, 30, 60, 110)];
            item.btnIndex = i;
...
- (void)clickAction:(YFActionSheetItem *)item
{
    if (_clickBlock) {
