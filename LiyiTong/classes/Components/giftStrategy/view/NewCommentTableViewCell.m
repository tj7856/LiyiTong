...
-(void)setup
{
    ...
    userName.text =[self.Resp.resp lastObject].name;
...
-(void)setResp:(NewResponse *)Resp
{
    ...
    for (SubResponse *resp in self.Resp.resp) {
        NSInteger index = [self.Resp.resp indexOfObject:resp];
        if (index ==0) {
            juli = 10.0;
        }
        else juli = 0;
