...
+(void)showHUDComponentDForView:(UIView *)view  title:(NSString *)title{
    UIImageView * animationImage=[[UIImageView alloc]initWithFrame:CGRectMake(0,0, 60, 60)];
    NSMutableArray * imageArray=[[NSMutableArray alloc]initWithCapacity:15];
    for (int i = 0; i<10; i++)
    {
        NSString * string = [NSString stringWithFormat:@"t%d.tiff",i+1];
...
+(BOOL)hideHUDSForView:(UIView *)view animated:(BOOL)animated{
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
